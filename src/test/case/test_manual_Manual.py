    ...
    def test_manual_Manual(self):
        ...
        for d in datas:
            with self.subTest(data=d):
                ...
                self.page.Resourcemanagement(Specifications,str(d['price_a']))
                time.sleep(1)
                ...
                order_no=self.page.orderadd(Specifications)
                time.sleep(1)
...
if __name__ == '__main__':
    report = REPORT_PATH + '\\test_manual_Manual.html'
    with open(report,'wb') as f:
        runner = HTMLTestRunner(f,verbosity=2,title='自动化测试报告',description='测试用例执行情况:')
        runner.run(Testorder('test_manual_Manual'))